...
func TestDetectPatterns_ShootingStar(t *testing.T) {
	...
	candles := []domain.Candle{
		candle(90, 92, 89, 91),  // pp: green
		candle(91, 93, 90, 92),  // p: green (uptrend)
		candle(92, 100, 92, 93), // c: tiny body, huge upper wick
	}
	sigs := DetectPatterns(candles, domain.Timeframe1h)
	found := false
	for _, s := range sigs {
...
func TestDetectPatterns_InsufficientCandles(t *testing.T) {
	sigs := DetectPatterns([]domain.Candle{candle(100, 101, 99, 100)}, domain.Timeframe1h)
	if len(sigs) != 0 {
