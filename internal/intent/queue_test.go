...
func TestTick_Frontrun_DoesNotFireBeforeInterval(t *testing.T) {
	...
	q.Tick(context.Background(), scheduler.WindowFrontrun)
...
func TestTick_Frontrun_FiresAfterInterval(t *testing.T) {
	...
	q := NewQueue(exec, 1*time.Millisecond) // tiny interval so it elapses immediately
	q.Enqueue(newIntent("PEPE", domain.EntryModeFrontrun, 82))
...
func TestTick_Frontrun_PicksHighestConfidence(t *testing.T) {
	rec := &recorder{}
	q := NewQueue(rec.exec, 1*time.Millisecond)
...
func TestTick_Frontrun_DoesNotFireTwiceWithinInterval(t *testing.T) {
	...
	q := NewQueue(exec, 1*time.Millisecond)
	q.Enqueue(newIntent("A", domain.EntryModeFrontrun, 80))
	q.Enqueue(newIntent("B", domain.EntryModeFrontrun, 75))

	time.Sleep(5 * time.Millisecond)
	q.Tick(context.Background(), scheduler.WindowFrontrun) // fires A (highest)
	q.Tick(context.Background(), scheduler.WindowFrontrun) // interval not yet elapsed again → no fire
...
func TestTick_LastMinute_FiresOnWindowEntry(t *testing.T) {
	rec := &recorder{}
	q := NewQueue(rec.exec, 5*time.Minute)
	q.Enqueue(newIntent("PEPE", domain.EntryModeLastMinute, 78))

	q.Tick(context.Background(), scheduler.WindowLastMinute)
...
func TestTick_LastMinute_FiresOnlyOnce(t *testing.T) {
	...
	q.Tick(context.Background(), scheduler.WindowLastMinute)
	q.Tick(context.Background(), scheduler.WindowLastMinute) // second tick same window
	q.Tick(context.Background(), scheduler.WindowLastMinute) // third tick
...
func TestTick_After_FiresOnWindowEntry(t *testing.T) {
	...
	q.Tick(context.Background(), scheduler.WindowAfter)
...
func TestTick_LastMinuteFire_DoesNotBlockAfterWindow(t *testing.T) {
	...
	q.Tick(context.Background(), scheduler.WindowLastMinute) // fires LM_COIN
	q.Tick(context.Background(), scheduler.WindowAfter)      // fires AFTER_COIN independently
...
func TestTick_FrontrunFire_DoesNotBlockLastMinuteOrAfter(t *testing.T) {
	...
	time.Sleep(5 * time.Millisecond)
	q.Tick(context.Background(), scheduler.WindowFrontrun)   // fires FR_COIN
	q.Tick(context.Background(), scheduler.WindowLastMinute) // fires LM_COIN independently
	q.Tick(context.Background(), scheduler.WindowAfter)      // fires AFTER_COIN independently
...
func TestClearAfterSettlement_ResetsWindowFiredFlags(t *testing.T) {
	...
	q.Tick(context.Background(), scheduler.WindowLastMinute) // fires A
	if fired.Load() != 1 {
...
func TestClearAfterSettlement_ResetsFrontrunTimer(t *testing.T) {
	...
	q.Enqueue(newIntent("A", domain.EntryModeFrontrun, 80))
	time.Sleep(5 * time.Millisecond)
	q.Tick(context.Background(), scheduler.WindowFrontrun) // fires, sets lastFrontrunExec
	if fired.Load() != 1 {
		t.Fatal("should have fired once")
	}
